         ...
         <div class="modal-body">
            <form id="form-filter">
               <div class="form-group">
                  ...
                  <select id="filter_phone" name="phone_number" class="form-control choose">
                    <option value="">----</option>
                     <?php
      ...
      <div class="modal-content">
         ...
         <form id="form-create">
            ...
            <div class="modal-body">
               <div class="form-group">
                  <label>Phone Number</label>
                  <select name="phone_number" class="form-control choose">
                     <option value=""></option>
                     <?php
                           $data = $this->db->get('simcard')->result_array();
                           foreach ($data as $value) {
               ...
               <div class="form-group">
                  <label>Email</label>
                  <select name="email" class="form-control choose">
                     <option value=""></option>
                     <?php
      ...
      <div class="modal-content">
         ...
         <form id="form-modify">
            <div class="modal-body">
               ...
               <div class="form-group">
                  <label>Phone Number</label>
                  <select id="phon_select" name="phone_number" class="form-control choose">
                     <?php
                           $data = $this->db->get('simcard')->result_array();
                           foreach ($data as $value) {
                              echo '<option value="'.$value['phone_number'].'">'.$value['phone_number'].'</option>';
               ...
               <div class="form-group">
                  <label>User</label>
                  <select id="user_select" name="user" class="form-control choose">
                     <?php
               ...
               <div class="form-group">
                  <label>Email</label>
                  <select id="email_select" name="email" class="form-control choose">
                     <?php
                           $data = $this->db->get('email')->result_array();
                           foreach ($data as $value) {
                              echo '<option value="'.$value['id'].'">'.$value['email'].'</option>';
...
<script type="text/javascript">
   ...
   $(function(){ 
      ...
      $(this).on('submit', '#form-create', function(e) {
         ...
         $(this).ajaxSubmit({
                type : "POST",
                url  : "<?php echo base_url('index.php/twitter/input_twitter')?>",
                dataType : "JSON",
                data : {
      ...
      $(this).on('submit', '#form-modify-password', function(e) {
         ...
         $(this).ajaxSubmit({
            type : "POST",
            url  : "<?php echo base_url('index.php/twitter/change_password')?>",
            dataType : "JSON",
            data:{
               'csrf_token_nalda' : $('#csrf').val()
            },
      ...
      $(this).on('click','.btn-edit',function(e){
            ...
            $.ajax({
                   type : "GET",
                   url  : "<?php echo base_url('index.php/twitter/ambil_edit')?>",
                   dataType : "JSON",
                   data : {
                     id:id
                   },
                  success: function(r){
                     form_edit.find('input[name="id"]').val(r.detail_twitter.id);
                     form_edit.find('input[name="display_name"]').val(r.detail_twitter.display_name);
                     form_edit.find('input[name="screen_name"]').val(r.detail_twitter.screen_name);
                     $('#status').val(r.detail_twitter.status);
                     $('#email_select').select2('val',r.detail_twitter.email_id);
                     $('#user_select').select2('val',r.detail_twitter.user_id);
                     $('#phon_select').select2('val', r.detail_twitter.phone_number);
                  }
            });
      ...
      $(this).on('submit', '#form-modify', function(e) {
            ...
            $(this).ajaxSubmit({
                   type : "POST",
                   url  : "<?php echo base_url('index.php/twitter/edit')?>",
                   dataType : "JSON",
                   data : {
                        'csrf_token_nalda' : $('#csrf').val()
                   },
      ...
      $(this).on('click','.item_hapus',function(e){
         ...
         $(this).ajaxSubmit({
            type : "GET",
                url  : "<?php echo base_url('index.php/twitter/hapus')?>",
                dataType : "JSON",
                data : {
                  no:id
                },
      ...
      $.fn.render_data  = function(params){
         ...
         ajaxManager.addReq({
            type : "GET",
            url : "<?php echo base_url('index.php/twitter/get_twitter')?>",
            async : false,
            dataType : "JSON",
            data : {
               offset:str.offset,
               ...
               if(data.total>0){
                  ...
                  var mail = "";
                     $.each(data.data, function(key,value){
                        ...
                        t+='<td>'+value.screen_name+'</td>';
                        $.each(value.email, function(k,v){
                           mail = v.email;
                        });
                        t+='<td>'+mail+'</td>';
                        t+='<td>'+value.follower+'</td>';
                        ...
                        t+='<td>'+usert+'</td>';
                        t+= '<div class="btn-group"><td><button class="btn btn-white btn-sm btn-edit" data-toggle="tooltip" data-original-title="Edit" data-id="'+value.id+'"><i class="fa fa-edit"></i></button>';
                        ...
                        t+= '<button class="btn btn-white btn-sm item_hapus" data-toggle="tooltip" data-original-title="Delete" data-id="'+value.id+'"><i class="fa fa-remove"></i></button></td></div>';
                        t+= '</tr>';
                     });
                  }else{
