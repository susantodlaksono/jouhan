...
<?php
	...
	$master_status_twitter = $this->db->select('a.*, b.name as status_name')->join('twitter_status as b', 'a.parent = b.id')->get('twitter_status_detail as a');
	if($master_status_twitter->num_rows() > 0){
		foreach ($master_status_twitter->result_array() as $key => $value) {
			$status_twitter[$value['status_name']][$value['id']] = $value['name'];
		}
	}else{
		$status_twitter = null;
	}
   ...
   <div class="col-md-12">
      <div class="panel panel-primary" data-collapsed="0">
         <div class= "panel-heading">
            <div class="panel-title"><i class="fa fa-list"></i> List Twitter Account</div>
            <div class="panel-options" style="width: auto;border:0px solid black;">
               <div class="form-group" style="margin-top: 8px;margin-bottom: 5px;">
                  <!-- <div class="btn-group dropdown-default"> 
                     <a class="btn btn-white btn-sm" href="<?php echo site_url() ?>twitter/download_format"><i class="fa fa-download"></i> Migration</a>
                  </div> -->  
                  <div class="btn-group dropdown-default"> 
                  ...
                  <div class="btn-group dropdown-default"> 
                	...
                	<div class="btn-group dropdown-default"> 
                     <button class="btn btn-white btn-sm" data-toggle="modal" data-target="#modal-migrasi-client"><i class="fa fa-upload"></i> Bulk</button>
                  </div>  
                  <div class="btn-group dropdown-default"> 
                  ...
                  <div class="btn-group dropdown-default"> 
                     ...
                     <ul class="dropdown-menu " style="width: auto;">
                        <?php
                        foreach ($status_twitter as $key => $value) {
                           echo '<li style="font-weight: bold;text-align: center;">'.$key.'</li>';
                           echo ' <div class="divider"></div>';
                           foreach ($value as $kk => $vv) {
                              echo '<li style="font-size: 13px;"><a href="#" class="bulk_action" data-mode="status" data-value="'.$kk.'">Set '.$vv.'</a></li>';
                           }
                           echo ' <div class="divider"></div>';
                        }
                        ?>
...
<div class="modal" id="modal-assign-client">
   <div class="modal-dialog" style="width: 35%;">
      <div class="modal-content">
         <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
            <h3 class="modal-title bold">Assign Client</h3>
         </div>
         <form id="form-assign-client">
            <div class="modal-body">
               ...
               <div class="form-group" style="display: none;">
                  ...
                  <select class="form-control choose" multiple="" name="module[twitter][]" disabled="" id="with_download_field">
                     <?php
...
<div class="modal" id="modal-raw">
   <div class="modal-dialog" style="width: 35%;">
      <div class="modal-content">
         <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
            <h3 class="modal-title bold"><i class="fa fa-upload"></i> Raw Data</h3>
         </div>
         <form method="post" action="<?php echo site_url('twitter/rawData') ?>" enctype="multipart/form-data">
            <div class="modal-body">
               <h5 class="text-center">Download the file format below</h5>
               <p class="text-muted text-center bold" style="margin-top: -5px;"><a href="<?php echo base_url() ?>format/ListPhoneNumber.xlsx" class="text-center"> ListPhoneNumber.xlsx</a></p>
               <div class="form-group">
            ...
            <div class="modal-footer">
               <button type="submit" class="btn btn-block btn-white"><i class="fa fa-download"></i> Download</button>
            </div>
         </form>
      </div>
   </div>
...
<div class="modal" id="modal-migrasi-client">
   <div class="modal-dialog" style="width: 35%;">
      <div class="modal-content">
         <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
         ...
         <form id="form-bulk-client">
            <div class="modal-body">
               ...
               <p class="text-muted text-center bold" style="margin-top: -5px;">
                  <a href="<?php echo site_url('twitter/bulkFormat/1') ?>" class="text-center"> PhoneNumberList.xlsx</a> |
                  <a href="<?php echo site_url('twitter/bulkFormat/2') ?>" class="text-center"> ScreenNameList.xlsx</a>
               </p>               
               <div class="form-group">
                  <label>Upload Format</label>
                  <input type="file" name="userfile" class="form-control">
   ...
   <div class="modal-dialog" style="width: 35%;">
      <div class="modal-content">
         ...
         <form id="form-migration">
            <div class="modal-body">
               <h5 class="text-center">Download the file format below</h5>
               <p class="text-muted text-center bold" style="margin-top: -5px;"><a href="<?php echo site_url() ?>/twitter/download_format" class="text-center"> Format Migration Twitter.xlsx</a></p>
               <div class="form-group">
                  <label>Upload Format</label>
                  <input type="file" name="userfile" class="form-control">
               </div>
      ...
      <div class="modal-content">
         ...
         <div class="modal-body">
            <form id="form-download-client" method="post" action="<?php echo site_url().'reporting/downloadByClient' ?>">
               <input type="hidden" name="cluster" value="twitter">
               <div class="form-group">
                  <label>Client</label>
                  ...
                  <div class="col-md-6">
                     <div class="form-group">
                        ...
                        <select id="filter_sort" class="form-control">
                           <option value="a.created_twitter" selected="">Created Twitter</option>
...
<div class="modal" id="modal-delete">
   <div class="modal-dialog" style="width: 40%;">
      <div class="modal-content">
         <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
            <h3 class="modal-title bold"><i class="fa fa-trash"></i> Delete</h3>
         </div>
         <form id="form-delete">
            ...
            <div class="modal-body">
               <div class="form-group">
                  ...
                  <select name="mode[]" class="form-control choose" multiple="">
                    <option value="1" selected="">Only Twitter</option>
               ...
               <div class="row">
                  <div class="col-md-4">
                     ...
                     <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="screen_name" required="" id="screen_name_create">
                     </div>
                     <div class="form-group">
                     ...
                     <div class="form-group">
                        ...
                        <div class="input-group">
                           <input type="text" class="form-control date-birthdate" data-formrender="#form-add">
                           <div class="input-group-addon"><i class="fa fa-calendar"></i></div>
                        </div>
                     </div>
                     <div class="form-group">
                        <label>Created Date <span class="text-require">(*)</span></label>
                        <input type="hidden" name="created_twitter">
                        <div class="input-group">
                	...
                	<div class="col-md-4">
                        ...
                        <select name="proxy_id" class="form-control choose">
                        	...
                        	foreach ($master_proxy->result_array() as $key => $value) {
                              $total_twitter = $this->db->where('proxy_id', $value['id'])->count_all_results('twitter');
                              $total_facebook = $this->db->where('proxy_id', $value['id'])->count_all_results('facebook');
                              $total_instagram = $this->db->where('proxy_id', $value['id'])->count_all_results('instagram');
                        		echo '<option value="'.$value['id'].'">'.$value['ip_address'].''.($value['port'] ? ':'.$value['port'].'' : '').' '.($value['network'] == 1 ? '(VPS)' : '(Proxy)').' (TW : '.$total_twitter.') (FB : '.$total_facebook.') (IG : '.$total_instagram.')</option>';
                        	}
                        	?>
                        </select>
                     </div>
   ...
   <div class="modal-dialog" style="width: 80%;">
      <div class="modal-content">
         ...
         <form id="form-edit">
            ...
            <input type="hidden" name="screen_name_before">
            <div class="modal-body">
               <div class="row">
                  <div class="col-md-4">
                     <div class="form-group">
                     ...
                     <div class="form-group">
                        <label>Screen Name <span class="text-require">(*)</span></label>
                        <input type="text" class="form-control" name="screen_name" required="" id="screen_name_edit">
                     </div>
                     <div class="form-group">
                        <label>Password <span class="text-require">(*)</span></label>
                        <input type="text" class="form-control" name="password" required="">
...
<script type="text/javascript">
   ...
   $(function () {
      ...
      $('#screen_name_create').keyup(function () { 
         this.value = this.value.replace(/[^a-zA-Z0-9_.-]/g,'');
      });

      $('#screen_name_edit').keyup(function () { 
         this.value = this.value.replace(/[^a-zA-Z0-9_.-]/g,'');
      });
                  ...
                  $.each(data.data, function(k,v){
                     t += '<tr>';
                        ...
                        t += '<td>';
                           t += '<b>'+v.pic+'</b></br>';
                           if(v.pic_updated){
                              t += '<a style="cursor:pointer" data-toggle="tooltip" data-placement="top" title="" data-original-title="By '+v.pic_updated+' at '+v.updated_date+'"><i class="fa fa-external-link"></i></a> at '+v.created_twitter+'';
                           }else{
                              t += 'at '+v.created_twitter+'';
                           }
                        t += '</td>';
                        t += '<td>';
                           if(v.phone_number){
                        ...
                        t += '</td>';
                        t += '<td>';
                           t += '<a href="http://www.twitter.com/'+v.screen_name+'" target="_blank">'+v.screen_name+'</a>';
                        t += '</td>';
                        t += '<td><button class="btn btn-xs btn-white btn-show-password" data-id="'+v.screen_name+'">Show Password</button><p class="show-password-'+v.screen_name+'" style="display:none;">'+(v.password ? v.password : '')+'</p></td>';
                        // t += '<td>';
                        // t += '<b>Apps id.</b> '+(v.apps_id ? v.apps_id : '')+'<br>';
                        // t += '<b>Apps Name.</b> '+(v.apps_name ? v.apps_name : '')+'';
                        // t += '</td>';
                        ...
                        t += '</td>';
                        t += '<td>'+v.status+'</td>'; 
                        t += '<td style="display:none;">'+(v.production_status ? v.production_status : '')+'</td>'; 
                        t += '<td>';
      ...
      $(this).on('click', '.btn-search-phone-number', function(e){
         ...
         if(phone !== ''){
            ajaxManager.addReq({
               type : "GET",
               url : site_url + 'twitter/find_phone',
               dataType : "JSON",
               data : {
                  phone : phone
               },
               ...
               success : function(r){
                  if(r.response){
                     if(r.response.registered_twitter == 0 && r.response.registered == 1){
                     ...
                     }else if(r.response.registered_twitter == 1){
                     ...
                     }else if(r.response.registered_twitter == 0 && r.response.registered == 0){
                        $('#'+render+'').find('input[name="phone_number"]').val('');
      ...
      $(this).on('submit', '#form-assign-client', function(e){
         ...
         $(this).ajaxSubmit({
            url  : site_url +'/twitter/assign_client',
            type : "POST",
            dataType : "JSON",
            data : {
      ...
      $(this).on('submit', '#form-bulk-client', function(e){
         var form = $(this);
         $(this).ajaxSubmit({
            url  : site_url +'/twitter/bulkTwitter',
            type : "POST",
            dataType : "JSON",
            data : {
               'csrf_token_nalda' : $('#csrf').val()
      ...
      $(this).on('submit', '#form-migration', function(e){
         var form = $(this);
         $(this).ajaxSubmit({
            url  : site_url +'twitter/migration_all',
            type : "POST",
            dataType : "JSON",
            data : {
               'csrf_token_nalda' : $('#csrf').val()
      ...
      $(this).on('click', '.btn-dismiss-modal',function(e){
         $(".choose").select2("close");
      });
      ...
      $(this).on('click', '.btn-edit',function(e){
         ...
         ajaxManager.addReq({
            ...
            success: function(r){
               ...
               form.find('.filter_search_phone_number').val('');
               ...
               form.find('input[name="screen_name_before"]').val(r.response.screen_name);
               form.find('input[name="screen_name"]').val(r.response.screen_name);
               form.find('input[name="password"]').val(r.response.password);
               form.find('input[name="display_name"]').val(r.response.display_name);
               if(r.response.created_twitter){
                  form.find('.date-data').val(moment(r.response.created_twitter).format('DD/MMM/YYYY'));
                  form.find('input[name="created_twitter"]').val(r.response.created_twitter);
               }else{
                  form.find('.date-data').val('');
                  form.find('input[name="created_twitter"]').val('');
               }
               ...
               form.find('input[name="twitter_id"]').val(r.response.twitter_id);
               form.find('input[name="followers"]').val(r.response.followers);
               form.find('input[name="apps_id"]').val(r.response.apps_id);
               form.find('input[name="apps_name"]').val(r.response.apps_name);
               form.find('input[name="cookies"]').val(r.response.cookies);
               form.find('input[name="consumer_key"]').val(r.response.consumer_key);
               form.find('input[name="consumer_secret"]').val(r.response.consumer_secret);
               form.find('input[name="access_token"]').val(r.response.access_token);
               form.find('input[name="access_token_secret"]').val(r.response.access_token_secret);
      ...
      $(this).on('click', '.assign-client',function(e){
         $('#form-assign-client').resetForm();
         $('#with_download_field').prop("disabled", true);
         $('#form-assign-client').find('select[name="module[twitter][]"]').select2('val', '');
         $('#form-assign-client').find('select[name="assign_client"]').select2('val', '');
         $('#your-file').html('');
         e.preventDefault();
      });
      ...
      $('.bulk_action').on('click', function(e) {
         ...
         if (data.length > 0) {
            ajaxManager.addReq({
               url: site_url + '/twitter/bulk_action',
               type: 'POST',
               dataType: 'JSON',
               data: {
                  mode: mode,
      ...
      $.fn.render_picker = function (opt) {
         ...
         $('.date-data').on('apply.daterangepicker', function(ev, picker) {
            var formrender = $(this).data('formrender');
            $(this).val(picker.startDate.format('DD/MMM/YYYY'));
            $(formrender).find('input[name="created_twitter"]').val(picker.startDate.format('YYYY-MM-DD'));
         });
      }
