        ...
        <div class="col-md-12">
            <div class="panel panel-primary" data-collapsed="0">
                <div class="panel-heading">
                    <div class="panel-title"><i class="fa fa-list"></i> List Client </div>
    ...
    <div class="modal fade" id="myUpdate">
        <div class="modal-dialog" style="width: 30%;">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    <h4 class="modal-title"><i class="fa fa-edit"></i> Edit Clients </h4>
                </div>
                <form id="form-edit">
                    <input type="hidden" class="form-control" style="width: 350px;" name="id_edit">
                    <div class="modal-body">
                        <div class="form-group">
                            <label> Client Name </label>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="form-filter">
                       <div class="form-group">
                          <label>Keyword</label>
                          <input type="text" id="filter_client" class="form-control">
                       </div> 
                    </form>
                </div>
                <div class="modal-footer">
          ...
          $('.choose').select2();
            ...
            $.fn.render_data = function(params){
                var p = $.extend({
                    ...
                    search : $('#filter_client').val(),
                },params);

                ajaxManager.addReq({
                    url: site_url +'/master_client/render_data',
                    type: 'GET',
                    dataType: 'JSON',
                    data: {
                       offset : p.offset,
                    ...
                    success:function(r){
                        ...
                        if(r.total!=0){
                            $.each(r.response, function(k ,v) {
                                t += '<tr>';
                                    t += '<td>'+v.id+'</td>';
                                    t += '<td>'+v.client_name+'</td>';
                                    t += '<td>'+v.desc+'</td>';
                                    t += '<td><div class="btn-group"><button class="btn-edit btn btn-white btn-change-password btn-sm" data-toggle="tooltip" data-original-title="Edit" data-id="'+v.client_id+'"><i class="fa fa-edit"></i></button><button class="btn btn-white btn-sm btn_hps" data-toggle="tooltip" data-original-title="Delete" data-id="'+v.client_id+'"><i class="fa fa-remove"></i></button></div></td>';
                                t += '</tr>';
            ...
            $(this).on('submit','#form-add',function(e){
                var form=$(this);
                $(this).ajaxSubmit({
                    url  : site_url +'/master_client/add',
                    type : "POST",
                    dataType : "JSON",
                    data : {
            ...
            $(this).on('submit','#form-edit',function(e){
                ...
                $(this).ajaxSubmit({
                    type : "POST",
                    url  : site_url +'/master_client/update',
                    dataType : "JSON",
                    data : {
                        'csrf_token_nalda' : $('#csrf').val()
                    },
            ...
            $(this).on('click','.btn-edit',function(e){
                ...
                $.ajax({
                    type : "GET",
                    url  : site_url + '/master_client/getID',
                    dataType : "JSON",
                    data : {
                        update : id
                    },
                    success: function(r){
                        form_update.find('input[name="id_edit"]').val(r.list.id);
                        form_update.find('input[name="client"]').val(r.list.name);
            ...
            $(this).on('click', '.btn_hps',function(e){
                ...
                $(this).ajaxSubmit({
                    type: 'GET',
                    url  : site_url + '/master_client/remove',
                    dataType : "JSON",
                    data : {
                        delete:id
                    },
