...
$cell->setCellValue('F1', 'FB ID');
$cell->setCellValue('G1', 'URL');
...
foreach ($report->result_array() as $v) {
	...
	$cell->setCellValue('B'.$i, date('Y-m-d', strtotime($v['created_facebook'])));
	$cell->setCellValue('C'.$i, $v['phone_number']);
	$cell->setCellValue('D'.$i, $v['email']);
	$cell->setCellValue('E'.$i, $v['display_name']);
	$cell->setCellValue('F'.$i, $v['facebook_id']);
    $cell->setCellValue('G'.$i, $v['url']);
