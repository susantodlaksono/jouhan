...
if($report){
   foreach ($report as $key => $value) {
      ...
      foreach ($module[$key] as $k => $v) {
         ...
         $sheet->getStyle('A1:'.$alphabet[$totalfield].'1')->getFill()->getStartColor()->setARGB('95B3D7');
      ...
      foreach ($value as $k => $v) {
         ...
         foreach ($module[$key] as $kk => $vv) {
            if($vv == 'status'){
               if($v[$vv] == 0){
                  $sheet->setCellValue(''.$alphabet[$kk].''.$column.'', 'Not Connected');
               }else{
                  $sheet->setCellValue(''.$alphabet[$kk].''.$column.'', 'Connected');
               }
            }else{
