...
$cell->setCellValue('C1', 'URL');
$cell->setCellValue('D1', 'Create Date (Example : 2019-01-31)');
$cell->setCellValue('E1', 'Facebook ID');
$cell->setCellValue('F1', 'Password');
$cell->setCellValue('G1', 'Cookies');
$cell->setCellValue('H1', 'Access Token');
$cell->setCellValue('I1', 'Friends');
$cell->setCellValue('J1', 'Proxy ID');
$cell->setCellValue('K1', 'Client ID');
$cell->setCellValue('L1', 'Status ID');
$cell->setCellValue('M1', 'User ID');
$cell->setCellValue('N1', 'Info');
$cell->getStyle('A1:N1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
$cell->getStyle('A1:N1')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
$cell->getStyle('A1:N1')->getFont()->setBold(TRUE);
foreach(range('A','N') as $column_id) {
 	$cell->getColumnDimension($column_id)->setAutoSize(true);
}
$cell->getStyle('A1:N1')->applyFromArray(
	array(
    	'fill' => array(
...
foreach ($status as $k => $v) {
	$cell->setCellValue('P'.$i, $k);
	$cell->setCellValue('Q'.$i, $v);
	$i++;
}

$cell->setTitle('Master Data');

$filename = 'Format Migration Facebook';
