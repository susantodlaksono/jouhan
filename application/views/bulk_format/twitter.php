...
$cell->setCellValue('ZZ4', $mode);
if($mode == 1){
    $cell->setCellValue('A2', 'Phone Number');
}
if($mode == 2){
    $cell->setCellValue('A2', 'Screen Name');
}
$cell->setCellValue('B2', 'Client ID');
$cell->setCellValue('C2', 'Status ID');
$cell->setCellValue('D2', 'Proxy ID');
$cell->setCellValue('E2', 'Screen Name');
$cell->setCellValue('F2', 'Display Name');
$cell->setCellValue('G2', 'Password');

//kode aktivasi untuk format
$cell->setCellValue('ZZ3', '1');

$cell->getStyle('A1:G2')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
$cell->getStyle('A1:G2')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
$cell->getStyle('A1:G2')->getFont()->setBold(TRUE);
$cell->getStyle('A2:G2')->applyFromArray(
	array(
    	'fill' => array(
...
$cell->setCellValue('A1', 'Kosongkan Jika tidak ingin mengupdate Client ID/Status ID/Proxy ID/Screen Name/Display Name/Password');
$cell->mergeCells('A1:G1');
$cell->getStyle('A1:G1')->applyFromArray(
	array(
    	'fill' => array(
        'type' => PHPExcel_Style_Fill::FILL_SOLID,
...
if($mode == 2){
    $cell->setTitle('List Screen Name');
}
...
$i = 3;
foreach ($twitter_status_detail->result_array() as $v) {
    $cell->setCellValue('M'.$i, $v['id']);
    $cell->setCellValue('N'.$i, $v['name']);
    $i++;
}
