...
class M_Proxy extends CI_model{
	...
	public function get($mode, $params){
      ...
      $this->db->select('count(c.proxy_id) as total_twitter');
      $this->db->select('count(d.proxy_id) as total_facebook');
      $this->db->select('count(e.proxy_id) as total_instagram');
      $this->db->join('users as b', 'a.user_id = b.id', 'left');
      $this->db->join('twitter as c', 'a.id = c.proxy_id AND c.status IN(3,10)', 'left');
      $this->db->join('facebook as d', 'a.id = d.proxy_id AND d.status IN(1)', 'left');
      $this->db->join('instagram as e', 'a.id = e.proxy_id AND e.status IN(1)', 'left');
