...
class Bulk_uploader{
   ...
   public function bulkEmail($filename, $params){
      $excel_reader = $this->ci->excel_reader->read(NULL, './upload/', ''.$filename);
      if ($excel_reader['status']) {
         $objPHPExcel = $excel_reader['result'];
         $count = 0;
         ...
         foreach ($worksheet as $value) {
            if ($row >= 2) {
               ...
               if($phone_number){
                  $check = $this->ci->db->where('phone_number', $phone_number)->count_all_results('simcard');
                  if($check > 0){
                     $data[$i]['phone_number'] = $phone_number;
                     $data[$i]['new_phone_number'] = isset($value['B']) ? $this->check_character($value['B']) : NULL;
                     $data[$i]['email'] = isset($value['C']) ? $value['C'] : NULL;
                     $data[$i]['password'] = isset($value['D']) ? $value['D'] : NULL;
                     $data[$i]['status'] = isset($value['E']) ? $value['E'] : NULL;
                  }
               }
               $i++;
            }
         ...
         if($data){
            if($keycode == 1){
               foreach ($data as $v) {
                  $tmp = array();
                  if($v['phone_number']){
                     if($v['new_phone_number'] || $v['email'] || $v['password'] || $v['status']){

                        if($v['new_phone_number']){
                           $tmp['phone_number'] = $v['new_phone_number'];
                        }
      ...
      if ($excel_reader['status']) {
         ...
         foreach ($worksheet as $value) {
            if ($row >= 2) {
               ...
               $data[$i]['password'] = isset($value['G']) ? $value['G'] : NULL;
               $i++;
            }
            $row++;
         }
         if($data){
            if($keycode == 1){
               foreach ($data as $v) {
                  $tmp = array();
                  if($v['key_data']){
                     if($v['client_id'] || $v['status'] || $v['proxy_id'] || $v['screen_name'] || $v['display_name'] || $v['password']){
         ...
         if($data){
            if($keycode == 1){
               foreach ($data as $v) {
                  $tmp = array();
                  if($v['phone_number']){
                     if($v['client_id'] || $v['status'] || $v['proxy_id'] || $v['password']){
                        if($v['client_id'] && is_numeric($v['client_id'])){
                           $client_id = $this->ci->db->where('id', $v['client_id'])->count_all_results('client');
                           if($client_id > 0){
                              $tmp['client_id'] = $v['client_id'];
